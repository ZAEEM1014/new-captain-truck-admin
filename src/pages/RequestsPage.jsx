import { truckTypes } from '../pages/TrucksPage';
...
const RequestsPage = () => {
  ...
  const loadAvailableResourcesForDate = async (date, requiredTruckTypes = []) => {
    try {
      ...
      // If only one truck type is required, pass it to getAvailableTrucksForDate
      let truckTypeToFilter = null;
      if (Array.isArray(requiredTruckTypes) && requiredTruckTypes.length === 1) {
        truckTypeToFilter = requiredTruckTypes[0];
      } else if (typeof requiredTruckTypes === 'string') {
        truckTypeToFilter = requiredTruckTypes;
      }
      const availTrucks = await getAvailableTrucksForDate(trucks, date, truckTypeToFilter);
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {assignment.assignmentDate && availableDrivers
                        .filter(driver => {
                          // Exclude drivers already selected in other assignments
                          const alreadyAssignedDriverIds = assignments.filter((a, i) => i !== index).map(a => a.driverId);
                          return !alreadyAssignedDriverIds.includes(driver.id);
                        })
                        .map(driver => (
                  ...
                  <div>
                    ...
                    >
                      ...
                      {assignment.assignmentDate && availableTrucks
                        .filter(truck => {
                          // Only show trucks matching the requested type for this assignment index
                          let requestedType = null;
                          if (Array.isArray(selectedDispatch.trucksRequired)) {
                            requestedType = selectedDispatch.trucksRequired[index]?.truckType;
                          } else {
                            requestedType = selectedDispatch.trucksRequired?.truckType || selectedDispatch.trucksRequired;
                          }
                          // Map requestedType to the actual truckTypes value (enforce exact match)
                          let matchedType = truckTypes.find(t => {
                            return t.value === requestedType || t.value === requestedType?.replace(/_/g, '-').toLowerCase();
                          });
                          let normalizedRequestedType = matchedType ? matchedType.value : requestedType?.replace(/_/g, '-').toLowerCase();
                          if (index === 0) {
                            console.log('Requested truck type for assignment', index, ':', requestedType, 'Normalized:', normalizedRequestedType);
                            console.log('Available trucks:', availableTrucks.map(t => ({id: t.id, truckType: t.truckType, plateNumber: t.plateNumber})));
                          }
                          const alreadyAssignedTruckIds = assignments.filter((a, i) => i !== index).map(a => a.truckId);
                          // Allow partial match for truck type
                          return truck.truckType?.toLowerCase().includes(normalizedRequestedType) && !alreadyAssignedTruckIds.includes(truck.id);
                        })
                        .map(truck => (
