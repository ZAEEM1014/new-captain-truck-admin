...
export const notifyNewRequest = async (dispatchId, tripDetails) => {
  try {
    ...
    let customerName = tripDetails.customerName || 'Unknown Customer';
    
    if (customerName === 'Unknown Customer' && tripDetails.customerId) {
      try {
        // Try to get customer details from database
        const customersSnapshot = await getDocs(collection(db, 'customers'));
        const customer = customersSnapshot.docs.find(doc => 
          doc.data().customerId === tripDetails.customerId || doc.id === tripDetails.customerId
        );
        
        if (customer) {
          customerName = customer.data().name || customer.data().firstName || 'Customer';
        }
      } catch (error) {
